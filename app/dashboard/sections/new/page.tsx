...
export default function NewSectionPage({
  ...
  const resolvedSearchParams = use(searchParams) as { pageId?: string };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Get the page document reference
      const pageId = resolvedSearchParams.pageId || "ps" // Default to "ps" if no pageId provided
      const pageRef = doc(db, "pages", pageId)
      const pageSnap = await getDoc(pageRef)

      // Structure the new section data according to the Java model
      const newSection = {
        // title: section.title,
        ...
        order: parseInt(section.order.toString(), 10)  // Ensure it's a number
      }

      // If page document exists, update it. If not, create it
      if (pageSnap.exists()) {
        const existingSections = pageSnap.data()?.sections || {}
        
        // Update page document with the new section
        ...
        // Create new page document with the section
